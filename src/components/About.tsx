export default function About() {
  ...
  return (
    <section id="about" className="text-white flex flex-col md:w-3xl mx-auto px-4 py-4">
      ...
      <div style={{backgroundColor: 'rgb(100, 100, 100)'}} className='w-full h-[0.5px] block mb-2 rounded' aria-hidden='true'></div>

      <div className='grid grid-cols-1 md:grid-cols-2 gap-2 px-4'>
        <h3 className='col-span-full text-lg font-semibold mb-2'>Technical Skills</h3>
        {skills.map((skill, index) => (
          <article key={index} className='grid grid-cols-[1fr_3fr] gap-2 items-center'>
            <div style={{backgroundColor: 'rgb(200, 200, 200)'}} className='rounded-3xl glow font-medium text-sm w-fit px-2 py-0 text-black' role='term'>
              {skill.name}
            </div>
            <p className='text-white text-sm' role='definition'>{skill.explanation}</p>
          </article>
        ))}
      </div>
    </section>
